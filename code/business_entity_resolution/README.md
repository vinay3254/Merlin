...
`src/train_cli.py` and `src/infer_cli.py` are thin argparse wrappers around
`run_training` (`src/train.py`) and `run_inference` (`src/infer.py`). Because
they're invoked as `python -m src.<module>`, they must be run with **`code/business_entity_resolution/`
as the working directory** (so the `src` package resolves); the dataset and
output paths below are therefore given relative to that directory, pointing
back up to `student_resource/` and the desired output locations at the repo
root.
...
2. Run inference (reads `student_resource/dataset/test/*` and
   `model.joblib`, writes both output TSVs under `output/` at repo root):
...
(Absolute paths for `--dataset-dir`/`--model-path`/`--output-dir` work too,
and avoid relying on relative-path bookkeeping.)
